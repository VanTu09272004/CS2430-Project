# CS2430 – Programming Project 1

## Team Members
- Tai - Implementation Lead  
- Musfer - Communications Lead  
- Deysi Oliver - Verification Lead  

---

## Milestone 1 – Planning (Finished)

| Task | Owner | Status | Evidence |
|------|--------|---------|----------|
| Choose programming language (Java) | Team | Done | Team discussion |
| Create GitHub repository | Tai | Done | VanTu09272004/CS2430-Project: CS 2430 Group Project - Spring 2026 |
| Assign roles | Team | Done | Canvas messages |
| Create project plan file | Musfer | Done | Commit adding ProjectPlan.md |

---

## Milestone 2 – Part 1 (Ordinary Sets)

| Task | Owner | Status | Evidence |
|------|--------|---------|----------|
| Create Boolean array structure | Tai | In Progress | — |
| Implement union operation | Tai | Not Started | — |
| Implement intersection operation | Tai | Not Started | — |
| Create 2-3 test cases + 1 edge case | Deysi | Not Started | — |
| Write design notes | Musfer | Not Started | — |

---

## Milestone 3 – Part 2 (Multisets)

| Task | Owner | Status | Evidence |
|------|--------|---------|----------|
| Design multiset using HashMap | Tai | Not Started | — |
| Implement multiset operations | Tai | Not Started | — |
| Create multiset test cases | Deysi | Not Started | — |
| Update README and report | Musfer | Not Started | — |

---

## Current Status
- Repository is created.
- Roles are confirmed.
- Part 1 is starting.
